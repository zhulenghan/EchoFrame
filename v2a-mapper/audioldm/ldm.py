...
class LatentDiffusion(DDPM):
    ...
    #                 bs=None,'
        ...
        if cond_key != self.first_stage_key:
            xc = super().get_input(batch, cond_key)[0]

        if not self.cond_stage_trainable or force_c_encode:
            # print(f{xc.shape=})
            # clip = calculate_clip(xc)
            # with torch.no_grad():
            #     c = v2a_mapper_model(torch.tensor(clip)).to(self.device)
            c = xc
        ...
        **kwargs,
        ...
        with self.ema_scope("Generate"):
            for batch in batchs:
                ...
                image = super().get_input(batch, "image") # 从batch里面取出image

                # Generate multiple samples
                batch_size = z.shape[0] * n_candidate_gen_per_text
                c = torch.cat([c] * n_candidate_gen_per_text, dim=0).unsqueeze(1)
                image = image * n_candidate_gen_per_text
                ...
                if waveform.shape[0] > 1:
                    similarity = self.cond_stage_model.cos_similarity(
                        torch.FloatTensor(waveform).squeeze(1), c
                    )
